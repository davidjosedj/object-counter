...
class HuggingFaceObjectDetector(ObjectDetector):
    """
    Object detector using Facebook's DETR model (PyTorch) from Hugging Face.
    This demonstrates support for a different deep learning framework (Task 6b).
    Original app uses TensorFlow Serving — this adapter uses PyTorch.
    Model auto-downloads on first run (~50MB).
    """

    def __init__(self, model: str = "facebook/detr-resnet-50"):
        self.__detector = pipeline("object-detection", model=model)

    def predict(self, image: BinaryIO) -> List[Prediction]:
        pil_image = Image.open(image).convert("RGB")
        width, height = pil_image.size
        raw_results = self.__detector(pil_image)
        return [self.__to_prediction(r, width, height) for r in raw_results]

    @staticmethod
    def __to_prediction(result: dict, width: int, height: int) -> Prediction:
        box = result['box']
        return Prediction(
